...
// modified for use in astrolib (by delphi29@excite.com, Oct 2000),
// because globals cannot be written from shared libraries;
// getMathLibRef() is implemented in glue.c
...
double sin(double x) {
	double result;
	//GetMathLibRef();
	//if (GetMathLibRef()==sysInvalidRefNum) { MathLibSin(GetMathLibRef(), x, &result); }
